...
def parse_time(time):
    '''
    Parse timestamp string extracted from file and cast it to number,
    the timestamp has to be in the UNIX universal time format.
    '''
    time = int(time)
    if time < 0:
...
def parse_coordinate(coord):
    '''
    Parse latitude or longitude coordinate string extracted from file and cast
    it to number, the coordinates has to be in the ISO 6709 string Annex H
    format.
    '''
    coord = float(coord)
...
def build_nmea_gpgaa_string(time, lat, lng, alt):
    ...
    msg = ('GP', 'GGA', (timestamp_to_gpstime(time),
                         ...
                         '', '0000')) # Time since DGPS last update and DGPS id
...
def main(flight_path_file, baudrate, tty, freq):
    ...
    lats, lngs, alts = interpolate_coordinates(times, times_extended, lats,
                                               lngs, alts)
